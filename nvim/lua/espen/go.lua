...
gohydra = Hydra({
  ...
  config = {
    ...
    hint = {
      ...
      funcs = {
        tags = function()
          return go_test_tags_string
        end,
        run = function()
          return go_test_run
        end,
        verbose = function()
          return flag(go_test_verbose)
        end,
        raw = function()
          return flag(raw_output_flag)
        end,
        short = function()
          return flag(go_test_short)
        end,
        env = function()
          return string.sub(go_test_env_string, 1, 20)
        end,
        test_home = function()
          return go_test_home
        end,
  ...
  heads = {
    {
      "s",
      function()
        go_test_short = not go_test_short
      end,
      { desc = "short" },
    },
    {
      "v",
      function()
        go_test_verbose = not go_test_verbose
      end,
      { desc = "raw output" },
    },
    {
      "o",
      function()
        raw_output_flag = not raw_output_flag
      end,
      { desc = "verbose" },
    },
    {
      "h",
      function()
        M.input("Home directory for go test", go_test_home, function(val)
          go_test_home = val
        end)
      end,
      { exit = true, desc = "set go test home directory" },
    },
    {
      "r",
      function()
        M.input("go test -run=", go_test_run, function(val)
          go_test_run = val
        end)
      end,
      { exit = true, desc = "set go run tag" },
    },
    {
      "e",
      function()
        M.input("environment variables", go_test_env_string, function(val)
          go_test_env_string = val
        end)
      end,
      { exit = true, desc = "set environment variabls for go test" },
    },
    {
      "t",
      function()
        M.input("go test tags", go_test_tags_string, function(val)
          go_test_tags_string = val
        end)
      end,
      { exit = true, desc = "set go test tag" },
    },
    { "q", nil, { exit = true } },
  },
})
